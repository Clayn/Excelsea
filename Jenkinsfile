    ...
    dir('excelsea') {
        ...
        stage('Reporting') {
            if (isUnix()) {
                ...
                sh "'${mvnHome}/bin/mvn' compile -DreleaseDeploymentRepository=clayn::default::${docu_dir} -DskipTests site:deploy"
            } else {
                bat(/"${mvnHome}\bin\mvn" compile -DskipTests site/)
                bat(/"${mvnHome}\bin\mvn" compile -DreleaseDeploymentRepository=clayn::default::${docu_dir} -DskipTests site:deploy/)
            }
        }
        stage('Results') {
